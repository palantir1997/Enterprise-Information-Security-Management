...
> 📝 메모에 정확한 페이로드가 안 적혀있어요 — 다음에 다시 실습하실 때 #2와 같은 방식(스크립트 컨텍스트인지, HTML 컨텍스트인지 먼저 F12로 확인)으로 접근해서 캡처해두면 좋습니다.
...
> ⚠️ 메모가 다소 축약되어 있어 정확한 메커니즘을 100% 확정하기는 어렵습니다. 가능성이 높은 두 가지로 추정됩니다:
> 1. **기본/약한 계정 노출**: bee-box는 학습용으로 phpMyAdmin에 root 계정이 비밀번호 없이(또는 매우 단순한 비밀번호로) 설정되어 있는 경우가 많아, 별도 인증 우회 기법 없이도 그냥 들어가지는 케이스
> 2. **URL Userinfo(`@`) 트릭을 이용한 검증 우회**: `http://신뢰된주소@공격자주소` 형태로 URL을 구성하면, 일부 단순한 Referer/호스트 검증 로직이 앞부분(`신뢰된주소`)만 보고 통과시켜버리는 경우가 있습니다.
>
> 다음 실습 때 phpMyAdmin 접근 직전의 요청을 Burp History에서 다시 캡처해서, 정확히 어떤 헤더/파라미터로 통과됐는지 확인해두면 보고서 작성 시 훨씬 명확해집니다.
