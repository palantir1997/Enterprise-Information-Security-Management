...
오늘 VulnHub 쪽은 처음엔 "다른 사람들 라이트업이랑 똑같이 했는데 왜 나만 안 되지"로 한참 막혔던 구간인데, 알고 보니 **이 VM(또는 미러 버전)이 원본 챌린지의 업로드 저장 로직과 미묘하게 달랐던 것**이 원인이었습니다. 확장자를 아무리 바꿔서 업로드해도 서버가 항상 `.png`로만 저장해버리는 걸 Burp 응답(`Content-Location`, `Vary: negotiate`)으로 확인한 게 결정적 단서였고, 결국 "필터를 통과한 다음, 서버가 실제로 디스크에 쓰기 직전"의 그 순간을 Burp Intercept로 직접 비집고 들어가 파일명을 조작하는 방식으로 뚫었습니다.

**오늘 배운 것**: 라이트업/블로그는 "정답이 어떻게 나왔는지"의 결과만 깔끔하게 보여주기 때문에, 환경이 조금만 달라도 그대로 따라 해선 안 통할 수 있다는 것. 그럴 때는 **블로그를 더 찾기보다, 지금 내 환경의 응답 헤더(Content-Type, Vary, Content-Location 등)를 직접 읽고 "서버가 실제로 뭘 하고 있는지"를 역으로 추론하는 게** 훨씬 빠른 길이라는 걸 직접 체감한 하루였습니다.
